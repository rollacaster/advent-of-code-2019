...
(defn compute-path [instructions]
  (reduce
   (fn [path [dir mag]]
     (reduce
      (fn [path i] (conj path (step (first path) dir)))
      path
      (range mag)))
   '([0 0])
   instructions))

(defn parse-wire-instructions [wire-string]
  (map (fn [[a & b]] [(str a) (Integer/parseInt (apply str b))])
                (s/split
                 wire-string
                 #",")))

(defn min-distance [instructions1 instructions2]
  (let [wire1 (parse-wire-instructions instructions1)
        wire2 (parse-wire-instructions instructions2)
        path1 (compute-path wire1)
        path2 (compute-path wire2)
        crossings (remove #(= [0 0] %) (intersection (set path1) (set path2)))]
    (map
     (fn [crossing]
       (println "crossing" crossing)
       (let [steps (take-while
                    (fn [step] (not (= step crossing)))
                    path1)]
         (println "steps" steps)
         steps))
     crossings)))
